@foreach ($list->mapels as $item)
    <div class="col-xl-4 col-md-6 mb-4">
        <div class="card border-left-info shadow h-100 py-2">
            @include('pages.siswa.includes.route', ['route' => route('detail.tugas', [$item->id, strtolower(\Str::slug($secroute ?? $item->nama))])])
                    ...
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2"><span class="badge badge-info float-right">{{ $item->tugas_sudah }}/{{ ($item->tugas_sudah + $item->tugas_belum) }} Tugas</span>
                            ...
                            <div class="justify-content-between">
                                @php
                                    $prog = $item->tugas_sudah + $item->tugas_belum;
                                    $hasil = $prog == 0 ? '0' : $item->tugas_sudah/$prog * 100;
                                @endphp
                                <div class="h5 font-weight-bold text-gray-800 float-right" style="margin-top: -7px;">{{ $hasil  }}%</div>
